...
import { ChartItem, Position, Style } from '@/types/draggable-panel';
import './index.sass';
...
export default markRaw(
  defineComponent({
    ...
    emits: ['canvas-scale', 'canvas-drop', 'char-click'],
    setup(props, ctx) {
      ...
      const pointStyleComputed = computed(() => {
        return (chartItem: ChartItem) => {
          ...
          const display =
            selectedChart.value === chartItem.id && !props.lock
              ? 'block'
              : 'none';
          return {
            ...{
      ...
      const scaleByWheel = (event: WheelEvent) => {
        ...
        canvasX.value -=
          (ratio - 1) * (event.clientX - canvasX.value) - origin.x;
        canvasY.value -=
          (ratio - 1) * (event.clientY - canvasY.value) - origin.y;
      };
      ...
      const realScale = () => {
        ...
        const offsetWidth: number | undefined = container.value?.offsetWidth;
      ...
      const panelPointerDown = (event: PointerEvent) => {
        if (
          event.srcElement === container.value ||
          event.srcElement === canvas.value
        )
          selectedChart.value = '';
      };
      ...
      const chartMovePointerDown = (event: PointerEvent, chart: ChartItem) => {
        ...
        selectedChart.value = chart.id as string;
        ...
        document.onpointermove = (ev: PointerEvent) => {
          ...
          movingChart.value.x = Math.floor(newX);
          movingChart.value.y = Math.floor(newY);
        };
      };
      ...
      const clickChart = (event: PointerEvent, chart: ChartItem) => {
        selectedChart.value = chart.id!;
        console.log(selectedChart.value);
        ctx.emit('char-click', event, chart);
      };
       ...
       */
      const dragleaveCanvas = (event: { srcElement: HTMLElement | null }) => {
      ...
      function createResizableList(chartItem: ChartItem) {
        ...
        pointDirectionList.forEach((direction) => {
          resizableList.push(
            h('div', {
              class: ['resizable-point', direction],
              key: direction,
              onPointerdown: (event: DragEvent) =>
                resizePointerDown(event, chartItem, direction),
        ...
        lineDirectionList.forEach((direction) => {
          resizableList.push(
            h('div', {
              class: ['resizable-line', direction],
              key: direction,
              onPointerdown: (event: DragEvent) =>
                resizePointerDown(event, chartItem, direction),
              onPointerup: resizePointerUp,
      ...
      function createChartList() {
        ...
        props.data.forEach((item, index) => {
          const defaultSlot: any = ctx.slots.chart!({
            chart: item,
            index,
          });
            ...
            h(
              'div',
              {
                class: [
                  ...
                  selectedChart.value === item.id ? 'selected' : '',
                ],
                key: item.id,
                ...
                onPointerdown: (event: PointerEvent) => {
                  chartMovePointerDown(event, item);
                  clickChart(event, item);
                },
      ...
      return () => [
        h(
          'div',
          {
            class: ['draggable-panel', props.lock ? 'lock' : ''],
            tabindex: -1,
            ref: container,
            autofocus: true,
